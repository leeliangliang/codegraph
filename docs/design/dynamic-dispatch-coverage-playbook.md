...
| `scripts/agent-eval/probe-semantic-objc-indexstore.sh` | macOS real-IndexStore smoke matrix for the semantic ObjC/Swift/C/C++ overlay (clang/swiftc → helper → merger → DB → explore) |
...
| Apple IndexStore overlay | ObjC / Swift / C / C++ | compiler-resolved call/reference/type/index facts → CodeGraph semantic edges | Compiler index + merger | ✅ **real-IndexStore smoke matrix added** (`scripts/agent-eval/probe-semantic-objc-indexstore.sh`): tiny ObjC/C/C++/Swift fixtures are compiled with clang/swiftc `-index-store-path`, enriched through the real `codegraph-xchelper`, verified in SQLite, then surfaced through `codegraph_explore`. Covers ObjC dynamic dispatch (`runCounter → increment`), ObjC read/write data-flow (`increment → value`), `#import` graph, decl→def, C address-of/function-pointer (`setup → handler`), C++ virtual dispatch (`drive → tick`), and Swift async metadata + semantic call (`fetchValue → makeBox`). **VVMusic large mixed-iOS validation (2026-06-09):** existing DerivedData IndexStore enriched 178,236 symbols, 527,301 semantic edges, 273,091 data-flow refs, 439 selector/fn-ref edges, 34,401 include edges, and 14,726 decl→def edges in a clean copied DB. Real-repo probing caught an over-broad dynamic-dispatch fan-out (`BEConcentrationTask.initTask` line 50 → 20+ unrelated `initTask` overrides); callsite-specific `receivedBy` locations now narrow it to the concrete receiver (`BEFaceAlgorithmTask.initTask`) while preserving the same 1,611 dynamic callsites, reducing synthesized dynamic edges **4,260→2,842**. `codegraph_explore` surfaces the narrowed dynamic hop and Swift target-action edges (`setupUI → backBtnClick` / `pravicyCheckboxClicked`). This proves helper/merger/surfacing on real compiler output; it is still not the full small/medium/large agent A/B matrix. 🔬 remaining: broader real-repo A/B by language/framework, SwiftUI `body` flow, framework delegate callback edges, cross-language `mainFilesContainingFile(crossLanguage:)`, and general `returns`/`instantiates` semantic edges |
