 ...
 */
function spawnDetachedDaemon(root: string, semanticObjc?: unknown): void {
  ...
  try {
    const child = spawn(
      ...
      [
        ...
        '--path',
        root,
        ...(semanticObjc === undefined ? [] : ['--semantic-objc-config', JSON.stringify(semanticObjc)]),
      ],
      {
        detached: true,
        stdio,
...
export class MCPServer {
  ...
  private semanticObjc: unknown;

  constructor(projectPath?: string, options: { semanticObjc?: unknown } = {}) {
    this.projectPath = projectPath || null;
    this.semanticObjc = options.semanticObjc;
  }
  ...
  private async startDirect(reason: string): Promise<void> {
    ...
    this.engine = new MCPEngine({ semanticObjc: this.semanticObjc });
...
export { isSemanticObjcGraphIdle, resolveSemanticObjcStorePathForProject } from './engine';
