...
function extractObjcContainerName(node: SyntaxNode, source: string): string | undefined {
  if (node.type !== 'class_interface' && node.type !== 'class_implementation') {
    return undefined;
  }

  const header = source.substring(node.startIndex, Math.min(node.endIndex, node.startIndex + 512));
  const category = header.match(/@(interface|implementation)\s+([A-Za-z_]\w*)\s*\(\s*([A-Za-z_]\w*)\s*\)/);
  if (category?.[2] && category?.[3]) {
    return `${category[2]}(${category[3]})`;
  }

  const normal = header.match(/@(interface|implementation)\s+([A-Za-z_]\w*)/);
...
function extractObjcName(node: SyntaxNode, source: string): string | undefined {
  const containerName = extractObjcContainerName(node, source);
...
export const objcExtractor: LanguageExtractor = {
  ...
  // @interface declarations and @implementation-only classes/categories can both introduce containers.
  ...
  visitNode: (node, ctx: ExtractorContext) => {
    ...
    const className = extractObjcContainerName(node, ctx.source);
