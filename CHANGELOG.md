...
- The macOS Objective-C / Swift semantic-enrichment helper no longer balloons to many gigabytes of memory while indexing a large Xcode project — its footprint now stays flat for the whole run instead of growing with every record it emits, which previously could push the machine into heavy swapping.
